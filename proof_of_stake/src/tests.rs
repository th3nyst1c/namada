...
use crate::types::{
    ...
    RedelegatedBonds, RedelegatedBondsMap, ReverseOrdTokenAmount, SlashType,
...
fn test_merge_redelegated_bonds_map() {
    ...
    .collect::<HashMap<_, _>>();
    let bob_tom_map = vec![
        (tom_address.clone(), tom_map.clone()),
        (bob_address.clone(), bob_map.clone()),
    ]
    .into_iter()
    .collect::<HashMap<_, _>>();
    let alice_tom2_map = vec![
        (tom_address.clone(), tom_map_2.clone()),
        (alice_address.clone(), alice_map.clone()),
    ]
    .into_iter()
    .collect::<HashMap<_, _>>();
    let everyone = vec![
        (alice_address.clone(), alice_map.clone()),
        (bob_address.clone(), bob_map.clone()),
        (tom_address.clone(), comb_tom_map.clone()),
    ]
    .into_iter()
    .collect::<HashMap<_, _>>();
